...
const int N = 400; //Number of Data Samples goes through
...
int main(void){
    ...
    for(int i = 1; i < N+1; i++){
        ...
        if(i==2){
            Err1 = loss(ATB_1, ATA_1, shared_bT, shared_a, tr_BTB[0], true);
        }else{
            Err1 = loss(ATB_1, ATA_1, shared_bT, shared_a, tr_BTB[0], false);
        }
        if(l_seg < 2){
            Err2 = loss(ATB, ATA, shared_bT, shared_a, tr_BTB[1], true);
            l_seg += 1;
        }else{
            Err2 = loss(ATB, ATA, shared_bT, shared_a, tr_BTB[1], false);
            l_seg += 1;
        }
        if(Err1 > t){
