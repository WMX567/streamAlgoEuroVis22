...
// Save result
ofstream savedFile("Radiation400_saved.txt");
...
void reading(){
    for(int i = 2; i < N+1; ++i){
        unique_lock<mutex> lk(m_lock);
        cond.wait(lk,[]{return counter == threads_min.size()+threads_max.size();});
        int k = get_k(i);
...
bool check_hash(int j){

    return hash_max.find(j) == hash_max.end() && hash_min.find(j) == hash_min.end();
}
    ...
    for(int i = 1; i < N+1; i++){
        ...
        if(Err1 > t){
            ...
            for(int j = e_1; j <= e_2; j++){
                if(check_hash(j)){
                    hash_max.insert(pair<int,int>(j,1));
                    vector<int> solu;
                    solu.push_back(i-1);
                    threads_max.push_back(thread(greedy_construct, upper, pow(base, j),i,solu));
                }
            }
...
int main(void){
    ...
    {
        unique_lock<mutex> lk(m_lock);
        threads_min.push_back(thread(reading));
        threads_min.push_back(thread(compute_max));
        lk.unlock();
        cond.notify_all();
    }
    ...
    for(int i = 1; i < N+1; i++){
        ...
        if(Err2 > t){
            ...
            if(lower > Err2){
                ...
                for(int j= e_1; j <= e_2; j++){
                    if(check_hash(j)){
                        hash_min.insert(pair<int,int>(j,1));
                        zero_solu[zero_solu.size()-1] = i;
                        threads_min.push_back(thread(greedy_construct, Err2, pow(base, j), i+1, zero_solu));
                        update_counter();
